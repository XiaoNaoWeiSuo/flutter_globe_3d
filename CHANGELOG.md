...
* 相机聚焦 API (setCameraFocus): 允许开发者通过传入经纬度直接设置相机瞄准的焦点，极大地简化了初始视图的配置。

* 优化与修复

* 修复经纬度定位偏差: 修复了 setCameraFocus 方法中经度计算的相位偏移问题，确保相机能够准确瞄准目标经纬度（如上海）。

* 优化跟随相机光照: 在 EarthLightMode.followCamera 模式下，将光源从正对相机调整到相机的左上方（相对于相机坐标系），从而在地球的右下方创建更明显的阴影和晨昏线效果，增强了球* 体的立体感。

* 内部改进

* 统一了相机聚焦逻辑的坐标系统，使其与节点投影和光照计算的 +90.0 经度偏移保持一致。
